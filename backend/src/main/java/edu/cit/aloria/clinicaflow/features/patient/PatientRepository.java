...
public interface PatientRepository extends JpaRepository<PatientEntity, String> {
    ...
    // THIS NOW RETURNS ALL PATIENTS (removed WHERE clause)
    @Query("SELECT p FROM PatientEntity p ORDER BY p.queueNumber ASC")
    List<PatientEntity> findAllPatientsOrdered();
    
    // Keep this for waiting patients only if needed
    @Query("SELECT p FROM PatientEntity p WHERE p.status = 'waiting' ORDER BY p.queueNumber ASC")
    List<PatientEntity> findWaitingPatientsOnly();
