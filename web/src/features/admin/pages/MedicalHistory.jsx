...
const Icons = {
  ...
  Calendar: () => (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={C.purple} strokeWidth="1.8">
  ...
  PatientIcon: () => (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={C.green} strokeWidth="1.8">
      <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" />
      <circle cx="12" cy="7" r="4" />
    </svg>
  ),
  Search: () => (
    <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.75)" strokeWidth="2">
  ...
  Filter: () => (
    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="rgba(0,0,0,0.7)" strokeWidth="2">
...
function StatCard({ title, value, sub, icon: Icon }) {
  return (
    ...
    onMouseLeave={(e) => {
      ...
      <div>
        <div style={{ fontSize: 12, fontWeight: 600, color: 'rgba(0,0,0,0.7)' }}>{title}</div>
        <div style={{ fontSize: 32, fontWeight: 700, color: '#000', marginTop: 6 }}>{value}</div>
        <div style={{ fontSize: 10, color: C.text2, marginTop: 6 }}>{sub}</div>
      </div>
...
export default function MedicalHistory() {
  ...
  const filteredConsultations = consultations.filter(c => {
    ...
    const matchesDoctor = doctorFilter === 'All Doctors' || c.doctorName === doctorFilter;
    const matchesDiagnosis = diagnosisFilter === 'All Diagnoses' || c.diagnosis === diagnosisFilter;
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              <div style={{ fontSize: 26, fontWeight: 700, color: C.primary }}>Medical History</div>
              <div style={{ fontSize: 13, color: 'rgba(25,0,81,0.65)', marginTop: 6 }}>View past consultations and patient records</div>
            </div>
          </div>
          ...
          <div style={{
            ...
            <Icons.Search />
            <input type="text" placeholder="Search consultations..." value={search}
        ...
        <div style={{
          ...
          <div style={{ padding: '26px 27px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontSize: 20, fontWeight: 700, color: '#fff' }}>Consultation History</div>
              <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.85)', marginTop: 10 }}>{filteredConsultations.length} consultations found</div>
            </div>
            <div style={{ display: 'flex', gap: 12 }}>
              <DoctorFilter value={doctorFilter} onChange={setDoctorFilter} doctors={uniqueDoctors} />
              <DiagnosisFilter value={diagnosisFilter} onChange={setDiagnosisFilter} diagnoses={uniqueDiagnoses} />
            </div>
          </div>
          ...
          <div style={{ backgroundColor: '#f9fafb', borderTop: '1px solid rgba(255,255,255,0.2)' }}>
            <div style={{
              display: 'grid', gridTemplateColumns: '60px 1.5fr 80px 2fr 1.8fr 2fr 100px',
          ...
          {filteredConsultations.map((consultation, idx) => (
            <div key={consultation.consultationId} style={{
              backgroundColor: C.white, borderTop: idx === 0 ? 'none' : '1px solid rgba(0,0,0,0.06)',
              padding: '0 41px', height: 72, display: 'grid', gridTemplateColumns: '60px 1.5fr 80px 2fr 1.8fr 2fr 100px',
            ...
            onMouseLeave={(e) => e.currentTarget.style.backgroundColor = C.white}>
              ...
              <div><div style={{ fontSize: 14, fontWeight: 600, color: C.text1 }}>{consultation.patientName}</div></div>
              ...
              <div><DiagnosisBadge diagnosis={consultation.diagnosis} /></div>
              <div>
                <button onClick={() => handleViewDetails(consultation)} style={{
                  background: 'none', border: 'none', cursor: 'pointer', fontSize: 12,
                  fontWeight: 600, color: C.primary, display: 'flex', alignItems: 'center',
                  gap: 6, padding: '6px 12px', borderRadius: 6, transition: 'background-color 0.2s ease',
                }}
          ...
          {filteredConsultations.length === 0 && (
            <div style={{ padding: '48px', textAlign: 'center', backgroundColor: C.white }}>
              <div style={{ fontSize: 13, color: C.text2 }}>No consultations found</div>
            </div>
          )}
        </div>
      </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ padding: '24px 28px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ fontSize: 18, fontWeight: 700, color: C.text1 }}>{isEditing ? 'Edit Consultation' : 'Consultation Details'}</div>
              <div style={{ display: 'flex', gap: 8 }}>
                {!isEditing && (
                  <button onClick={handleEditClick} style={{
                    background: 'none', border: 'none', cursor: 'pointer', padding: '8px 12px',
                    borderRadius: 6, display: 'flex', alignItems: 'center', gap: 6, color: C.primary,
                    fontSize: 13, fontWeight: 600, transition: 'background-color 0.2s ease',
                  }}
                  onMouseEnter={(e) => e.currentTarget.style.backgroundColor = C.purpleBg}
                  onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}>
                    <Icons.Edit /> Edit
                ...
                <button onClick={() => handleDeleteConsultation(selectedConsultation)} style={{
                  background: 'none', border: 'none', cursor: 'pointer', padding: '8px 12px',
                  borderRadius: 6, display: 'flex', alignItems: 'center', gap: 6, color: C.red,
                  fontSize: 13, fontWeight: 600, transition: 'background-color 0.2s ease',
                }}
            ...
            {isEditing ? (
              <div style={{ padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div><label style={{ fontSize: 12, fontWeight: 600, color: C.text2 }}>Patient Name</label>
                  <input type="text" name="patientName" value={editFormData.patientName} onChange={handleEditInputChange}
                    style={{ width: '100%', height: 40, border: '1px solid #e2e8f0', borderRadius: 8, padding: '0 12px', fontSize: 13 }} />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <div><label>Age</label><input type="number" name="age" value={editFormData.age} onChange={handleEditInputChange}
                    style={{ width: '100%', height: 40, border: '1px solid #e2e8f0', borderRadius: 8, padding: '0 12px', fontSize: 13 }} /></div>
                ...
                <div><label>Doctor</label><input type="text" name="doctorName" value={editFormData.doctorName} onChange={handleEditInputChange}
                  style={{ width: '100%', height: 40, border: '1px solid #e2e8f0', borderRadius: 8, padding: '0 12px', fontSize: 13 }} /></div>
                <div><label>Date</label><input type="date" name="consultationDate" value={editFormData.consultationDate?.split('T')[0] || ''} onChange={handleEditInputChange}
                  style={{ width: '100%', height: 40, border: '1px solid #e2e8f0', borderRadius: 8, padding: '0 12px', fontSize: 13 }} /></div>
                <div><label>Symptoms</label><textarea name="symptoms" value={editFormData.symptoms} onChange={handleEditInputChange} rows={3}
                  style={{ width: '100%', border: '1px solid #e2e8f0', borderRadius: 8, padding: '10px 12px', fontSize: 13, resize: 'vertical' }} /></div>
                <div><label>Diagnosis</label><input type="text" name="diagnosis" value={editFormData.diagnosis} onChange={handleEditInputChange}
                  style={{ width: '100%', height: 40, border: '1px solid #e2e8f0', borderRadius: 8, padding: '0 12px', fontSize: 13 }} /></div>
                <div><label>Prescription</label><textarea name="prescription" value={editFormData.prescription} onChange={handleEditInputChange} rows={3}
                  style={{ width: '100%', border: '1px solid #e2e8f0', borderRadius: 8, padding: '10px 12px', fontSize: 13, resize: 'vertical' }} /></div>
                <div><label>Remarks</label><textarea name="remarks" value={editFormData.remarks} onChange={handleEditInputChange} rows={2}
                  style={{ width: '100%', border: '1px solid #e2e8f0', borderRadius: 8, padding: '10px 12px', fontSize: 13, resize: 'vertical' }} /></div>
              </div>
            ) : (
              <div style={{ padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div><label>Patient Name</label><div style={{ fontSize: 14, fontWeight: 500, color: C.text1 }}>{selectedConsultation.patientName}</div></div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <div><label>Age</label><div>{selectedConsultation.age || 'N/A'}</div></div>
                  <div><label>Gender</label><div>{selectedConsultation.gender || 'N/A'}</div></div>
                </div>
                <div><label>Date</label><div>{formatDate(selectedConsultation.consultationDate)}</div></div>
                <div><label>Doctor</label><div>{selectedConsultation.doctorName}</div></div>
                <div><label>Symptoms</label><div style={{ backgroundColor: '#f8fafc', padding: 16, borderRadius: 12 }}>{selectedConsultation.symptoms || 'No symptoms recorded'}</div></div>
                <div><label>Diagnosis</label><DiagnosisBadge diagnosis={selectedConsultation.diagnosis} /></div>
                <div><label>Prescription</label><div style={{ backgroundColor: '#f8fafc', padding: 16, borderRadius: 12 }}>{selectedConsultation.prescription || 'No prescription recorded'}</div></div>
                <div><label>Remarks</label><div style={{ backgroundColor: '#f8fafc', padding: 16, borderRadius: 12 }}>{selectedConsultation.remarks || 'No remarks'}</div></div>
              </div>
            )}

            <div style={{ padding: '16px 28px', borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
              <button onClick={() => { setSelectedConsultation(null); setIsEditing(false); }} style={{
                height: 40, padding: '0 24px', border: '1px solid #e2e8f0', borderRadius: 8,
                backgroundColor: C.white, color: C.text2, fontSize: 13, fontWeight: 600, cursor: 'pointer',
              }}>Cancel</button>
              {isEditing && (
                <button onClick={handleSaveEdit} style={{
                  height: 40, padding: '0 24px', border: 'none', borderRadius: 8,
                  backgroundColor: C.green, color: C.white, fontSize: 13, fontWeight: 600, cursor: 'pointer',
                }}>Save Changes</button>
              )}
            </div>
          </div>
